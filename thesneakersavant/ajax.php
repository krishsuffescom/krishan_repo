...
if($_REQUEST['do']=='veenomousbrand'){
	
	 $sql=mysql_query("SELECT * FROM `shoes_models` where brand_id='$_GET[id]'");
	...
	$message[0] = 'success';
	$message[1] = "<option value=''>Please Select</option>".$data;

	$message_arr = implode('|::|', $message);
	echo $message_arr;
}
if($_REQUEST['do']=='veenomousproduct'){
	
	 $sql=mysql_query("SELECT * FROM `product` where model_id='$_GET[id]'");
	while($row=mysql_fetch_assoc($sql)){
		$id  	=	$row['id'];
		$name	=	$row['name'];
		$data.= '<option value="'.$id.'">'.$name.'</option>';
