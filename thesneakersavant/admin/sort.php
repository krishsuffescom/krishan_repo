...
// If type is not 1 start fetching parent item
if($type<>1){
	$sql = "SELECT * from categories where relation='$fetch[relation]' order by sort ASC";
}
else{
	$sql = "SELECT * from categories where type='1' order by sort ASC";
}
...
<ul id="test-list">
	...
	while($fetch1 = mysql_fetch_assoc($exec)){
		echo '<li id="listItem_'.$fetch1[id].'" style="padding:2px;"><img src="images/arrow.png" alt="move" width="16" height="16" class="handle" /> '.$fetch1[name].'</a></li>';
		$i++;
	}
