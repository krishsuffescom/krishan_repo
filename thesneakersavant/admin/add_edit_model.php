      ...
      <div class="form5">
        <form id="add" name="add" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?1=1<?php echo $q_string; ?>" enctype="multipart/form-data">
            <table width="1150" border="0" cellpadding="5">
               <tr>                
                  <td align="right" class="label_form">Model name: </td>
                  <td><input type="text" name="name" class="input_text" value="<?php echo $dvar['name']; ?>"  /> 
				  <br class="clear" /></td>
              </tr>
			  
              <tr>
              <td valign="top" align="right" class="label_form">Brand : </td>
            <td>
                    <select name="brand_id" class="input_text brand_id" style="width: 250px;">
                        <option value="">Select...</option>
                         <?php $sql="SELECT * FROM `shoes_brand` ORDER BY id desc";
							$exec=mysql_query($sql);
							while($row=mysql_fetch_assoc($exec)){?>
								<option value="<?=$row['id']?>" <?php if($dvar['brand_id']==$row['id']){?> selected="selected" <?php } ?>><?=$row['name']?></option>
						<?php } ?>
                   </select>
            </tr>
           <tr>
